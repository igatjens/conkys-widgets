...
function conky_main()
    ...
    local posicion_y = conky_window.height - 5
...
function conky_dibujar_grafico( )
    local ancho = conky_window.width - get_graf_margen_derecho(  ) - get_graf_ram_margen_izquierdo( )

    if alto_grafico < 1 then
        alto_grafico = (conky_window.height - 20)
    end

    local codigo = "${goto "..get_graf_ram_margen_izquierdo( ).."}${voffset 8}${memgraph "..alto_grafico..","..ancho.." }${voffset -8}"
    return codigo
end

...
function conky_main()
    ...
    if not esta_hecho then
        conky_parse("${exec sed -i 's/own_window_transparent *no/own_window_transparent yes/' *.conf}")
        conky_parse("${exec sed -i 's/own_window_colour *....../own_window_colour 000000/' *.conf}")
        conky_parse("${exec sed -i 's/own_window_argb_value *.*/own_window_argb_value 255/' *.conf}")
    ...
    texto = "¡Listo! Fondo transparente aplicado."   
